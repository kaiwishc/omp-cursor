...
| Context handoff | Fresh by default; bootstrap requires explicit CLI/environment/user consent because it sends prior OMP context to cloud, which may include file contents, tool outputs, paths or skill references, environment values, or secrets. | **Implemented** |
| Cursor-managed environment | Explicit `cloud` / `pool` / `machine` selection; OMP does not automatically forward process environment values. | **Implemented** |
| OMP env forwarding and `.env` reads | Parsed reserved shapes fail preflight with Cursor-native environment guidance. | **Intentionally deferred/rejected** — avoids a parallel secret-management path. |
...
| Local resume | Default-on branch-scoped resume with strict session/tree/compaction/tool-surface/store identity and explicit cleanup. Persisted OMP sessions use per-session SQLite stores; fileless acquisitions use removable temporary stores. | **Implemented** |
...
Automatic provider startup in print/JSON/RPC does not prompt and fails closed when acknowledgement or required OMP-owned safety choices are absent. The `/cursor-runtime cloud` command is separate: it may request confirmation whenever the host exposes UI (`ctx.hasUI`), including an RPC host with UI. Project-local loading and explicit `--auto-approve` behavior are covered in `test/cursor-project-trust-contract.test.ts`.
...
| OMP env forwarding and `.env` reading | **Intentionally deferred/rejected** | `src/cursor-cloud-options.ts` rejects configured forwarding and directs users to Cursor-native environments; covered by `test/cursor-provider-cloud-env-validation.test.ts`. |
...
| No automatic injection or forwarding of the local OMP bridge, local MCP, `local.customTools`, local settings or skill metadata, or process environment values to cloud | **Implemented** | This excludes automatic metadata forwarding, not content explicitly included in consented bootstrap context. Anchors: `src/cursor-cloud-options.ts`, `src/cursor-provider-turn-prepare.ts`, `src/cursor-skill-tool.ts`; `test/cursor-provider-stream-config.test.ts`, `test/cursor-skill-tool.test.ts`. |
...
| Cursor-native SDK `agents` / OMP-subagent mapping | **Intentionally deferred/rejected** | SDK exposes `AgentOptions.agents`; no automatic OMP mapping is approved. Cursor may load native `.cursor/agents/*.md` from its own settings. |
...
| Cloud agent naming from OMP session title | **Implemented** | `src/cursor-provider-turn-prepare.ts`; `test/cursor-provider-stream-config.test.ts`. |
| Agent/run IDs, branch/PR, passive artifacts, and raw usage completion telemetry | **Implemented** | `src/cursor-cloud-reporting.ts`, `src/cursor-provider-turn-finalize.ts`; `test/cursor-provider-cloud-reporting.test.ts`, `test/cursor-cloud-reporting.test.ts`. Raw usage remains display-only and outside OMP accounting/transcript. |
...
| Automatic local force recovery | **Intentionally deferred/rejected** | Manual `--cursor-local-force` is implemented. Automatic recovery lacks OMP ownership, heartbeat/staleness proof, active-run status, stable idempotency, and cross-handle cleanup guarantees. |
| Feeding `RunResult.usage` into OMP message/context accounting | **Intentionally deferred/rejected** | Real local evidence showed full-agent-context values that poison compaction. OMP uses valid local `turn-ended` usage or bounded estimates; cloud raw usage stays display-only until its field semantics are captured. Coverage is in usage/provider tests and retained local compaction evidence. |
...
`test/cursor-project-trust-contract.test.ts` packs and extracts the extension, then launches the installed OMP CLI against that isolated provider with isolated agent/project directories and a project `.omp/cursor-sdk.json` in print, JSON, and RPC modes. A test-only wrapper in the extracted package records the canonical `session_start` scope and `resolveCursorProviderTurnConfig()` result. The matrix proves OMP's project-local package does not depend on a Pi trust store and that explicit `--auto-approve` allows recognized or standalone project config to select cloud while keeping acknowledgement out of project scope.
