...
- Build first: `npm run build` after any `src/` edit — the OMP manifest loads compiled `dist/`, so unbuilt runs validate stale code. (the cloud/steering/local-resume/provider-debug launchers rebuild automatically even when run directly with `node scripts/...`; `smoke:live`/`smoke:visual`/`smoke:isolated` build via their npm scripts; direct `omp -e .` invocations do not build.)
- Run from a clean working tree except for the intended branch diff.
- Use the local extension under test: `omp --auto-approve -e . --cursor-no-fast --model cursor-sdk/grok-4.6`.
...
```bash
export SMOKE_DIR="/tmp/omp-cursor-live-smoke-$(date +%Y%m%dT%H%M%S)"
mkdir -p "$SMOKE_DIR"
omp --version
npm ls @cursor/sdk @oh-my-pi/omptype @oh-my-pi/pi-coding-agent @oh-my-pi/pi-ai @oh-my-pi/pi-tui
omp models cursor-sdk
```

- Live OMP runs resolve the Cursor API key from the current run's `--api-key`, `CURSOR_API_KEY`, or the global OMP user config `~/.omp/agent/cursor-sdk.json`; no `auth.json` is read or seeded. For isolated runs, copy only `cursor-sdk.json` into the temporary `~/.omp/agent` directory or export `CURSOR_API_KEY`.
...
`npm run smoke:live` resolves `omp`, `node`, `npm`, `rg`, and `tmux` once in the parent shell, then runs all OMP shims with the resolved Node directory first on `PATH`. It clears inherited Cursor SDK event-debug env for every child OMP run. Isolated helper cases force `PI_CURSOR_SETTING_SOURCES=none`; the `default-settings` helper case explicitly unsets `PI_CURSOR_SETTING_SOURCES` so it exercises the default ambient setting-source path.
...
`npm run smoke:isolated` follows the same smoke-runner env contract as live/visual/steering helpers: pack-only work resolves only `node`, `npm`, and `env` from the parent shell and does not require `omp`; live checks then resolve `omp` and `rg`. It runs OMP/npm shims with the resolved Node directory first on `PATH`, clears Cursor SDK event-debug env, forces `PI_CURSOR_SETTING_SOURCES=none` for provider checks, and explicitly unsets `PI_CURSOR_SETTING_SOURCES` for install/list checks.
...
- `omp --version` reports OMP 18.1.15 for this cutover baseline.
- `npm ls` shows `@cursor/sdk@1.0.27` and local `@oh-my-pi/*@18.1.15` packages.
- `cursor-sdk/grok-4.6` appears in the model list.
- No Cursor key or auth token is printed.
- If neither `~/.omp/agent/cursor-sdk.json` `apiKey` nor `CURSOR_API_KEY` is available, stop and report the live smoke as blocked.
...
```bash
SESSION="omp-cursor-smoke-$(date +%s)"
tmux new-session -d -s "$SESSION" -x 120 -y 40 -- zsh -lc \
  "cd '$PWD' && PI_CURSOR_SETTING_SOURCES=none omp --auto-approve -e . --cursor-no-fast --model cursor-sdk/grok-4.6 --session-dir '$SMOKE_DIR/tui' --no-tools 'TUI smoke. Compute 19 + 23. Reply only with SUM=<number>.'"
```
...
- The run uses the OMP `--session-dir` storage directory; OMP assigns the session file/id.
...
This is the canonical inner-loop visual debug path for Cursor provider/runtime changes. It requires offscreen TUI visual inspection, not only JSONL or code review. Use OMP 18.1.15, `@cursor/sdk@1.0.27`, a fresh temporary session dir, Cursor SDK `plan` mode, native replay enabled, and the checked-in visual runner. The runner resolves `omp` by directly walking the parent `PATH`, uses `process.execPath` for Node, and prepends that Node directory for both prereq checks and tmux launches so `#!/usr/bin/env node` shims use the validated Node. The default matrix is native replay only: native replay registration is forced on, settings sources are `none`, the OMP bridge is off, overlapping built-in OMP tools are not exposed, and inherited Cursor SDK event-debug artifact env is cleared.

```bash
VISUAL_DIR="$(mktemp -d /tmp/omp-cursor-1016-visual.XXXXXX)"
VISUAL_ARGS=(
  --ext "$PWD"
  --cwd "$PWD"
  --out-dir "$VISUAL_DIR"
...
npm run smoke:visual -- "${VISUAL_ARGS[@]}" \
  --label grep-readme \
  --prompt 'Use only your grep/search tool to search ./README.md for the literal string "omp-cursor". Do not use shell, read, glob, find, ls, or list tools. Report only the first matching file path.'
...
- Native-looking read/search/find/list/shell/write/edit cards use intended OMP card styling.
...
```bash
PI_CURSOR_SETTING_SOURCES=none \
omp --auto-approve -e . --cursor-no-fast --cursor-mode plan --model cursor-sdk/grok-4.6 \
...
- No OMP active-tool or OMP plan-mode state is mutated merely because Cursor SDK mode is `plan`.
...
PI_CURSOR_PI_TOOL_BRIDGE=1 PI_CURSOR_PI_TOOL_BRIDGE_DEBUG=1 \
omp --auto-approve -e . --cursor-no-fast --model cursor-sdk/grok-4.6 \
  --session-dir "$SMOKE_DIR/bridge" \
  -p 'Bridge smoke. Do exactly two tool calls before answering: first call pi__read on ./package.json; second call pi__read on ./definitely-missing-omp-cursor-smoke-file.txt. Then answer: OK_NAME=<package name>; MISSING_RESULT=<error or success>. Do not use shell.' \
...
- stdout includes `OK_NAME=omp-cursor`.
...
- Persisted JSONL contains real OMP tool calls named `read`, matching `toolResult` messages, and final assistant output.
- Later assistant usage counts consumed tool-result input; no assistant usage has negative values.

## 7. Native replay cards without the OMP bridge
...
omp --auto-approve -e . --cursor-no-fast --model cursor-sdk/grok-4.6 \
  ...
  -p 'Native replay smoke. Use your Cursor file-reading capability to read ./README.md, then answer README_SEEN=yes if it contains omp-cursor.' \
...
- Persisted JSONL shows an assistant `toolUse` turn with a replayed `read` tool call, an OMP `read` `toolResult`, and a final assistant turn.
- Native replay is display-only: it must not re-run Cursor-side mutations or create duplicate OMP mutations.
...
- bridge/OMP tool call IDs derived from the run-safe ID
- hashed Cursor MCP call correlation IDs of the form `cursor-mcp-call-<8 hex chars>`
- exposed OMP/MCP tool name pairs
...
- Interrupting the run does not leave `sleep 30`, `SHOULD_NOT_PRINT`, `omp`, or bridge-related child processes running.
...
- All commands exit `0`.
- `npm pack --dry-run` includes all new runtime source files and the compiled `dist/` output, and excludes local smoke artifacts, sessions, package tarballs, `.env*`, `.omp/`, and `coverage/`.

## 12. Cleanup

```bash
tmux list-sessions | grep 'omp-cursor-smoke' || true
rm -rf "$SMOKE_DIR"
```
