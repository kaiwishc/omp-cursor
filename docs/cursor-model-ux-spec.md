...
- OMP Agent Hub children run headless worker sessions, but the shipped `cursor` subagent uses the existing display-only replay wrappers so reported Cursor host tools, configured MCP completions, and `SKILL.md` reads remain visible in its persisted transcript and Activity view. Ordinary print-mode main sessions remain text-first. Bridge `pi__*` calls continue through the real OMP execution path and are rendered once.
