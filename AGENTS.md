...
This repository is an OMP provider extension that registers Cursor SDK-backed models under the `cursor-sdk` provider. Agent work is successful when changes preserve OMP-native model/thinking/session behavior, keep Cursor API keys out of repo state and logs, and pass the local validation commands below.

## Repository map

- `src/index.ts` registers the OMP extension, provider, fallback warnings, Cursor runtime controls, native replay wrappers, question tool, and OMP tool bridge hooks.
- `src/model-discovery.ts` discovers Cursor models, builds OMP model metadata, stores per-model metadata, and defines fallback models.
...
- `src/cursor-session-scope.ts` owns OMP session cwd, session file/id/name/generation scope keys, and `session_start` / `session_info_changed` registration for session-agent pooling, cloud agent names, and debug grouping.
...
- `src/cursor-agents-context.ts` owns Cursor-model suppression of OMP `<project_context>` / `AGENTS.md` duplication and `PI_CURSOR_PRESERVE_PI_AGENTS_MD`; `src/cursor-agents-context-registration.ts` owns the static lifecycle registration for that suppression.
- `src/cursor-sdk-output-filter.ts` suppresses Cursor SDK integrator bootstrap noise from OMP's TUI.
...
- `src/cursor-usage-accounting.ts` owns OMP usage mapping from local turn-ended and billed `Agent.getUsage()` spend, plus post-compaction occupancy floors.
...
- `src/cursor-transcript-tool-specs.ts` owns per-tool transcript formatters and OMP display builders keyed by normalized tool name; its display implementation keys must match registry entries exactly (`CURSOR_TOOL_DISPLAY_SPEC_KEYS`).
...
- `src/cursor-pi-tool-bridge.ts` re-exports bridge registration and snapshot helpers; exposes active OMP tools to local Cursor agents through a per-run loopback MCP bridge.
...
- `src/cursor-pi-tool-bridge-run.ts` owns MCP transport setup, pending bridge calls, OMP tool dispatch, cancellation, and run lifecycle.
- `src/cursor-pi-tool-bridge-abort.ts` owns bridge OMP tool execution abort tracking and process signal handling.
- `src/cursor-pi-tool-bridge-diagnostics.ts` owns bridge debug diagnostics serialization and stderr logging.
- `src/cursor-pi-tool-bridge-mcp.ts` owns MCP name/schema conversion and OMP-to-MCP content helpers for the bridge.
...
- `src/cursor-question-tool.ts` owns the bridge-exposed `cursor_ask_question` OMP UI tool and the `omp-cursor:ask-question:blocked` wait-state event.
...
- `src/cursor-bridge-contract.ts` owns OMP bridge MCP description helpers and the exported full bridge contract text (bootstrap/manifest carry the user-facing contract; MCP descriptions use a one-line pointer).
...
- `test/helpers/pi-harness.ts` is the canonical fake OMP/extension harness (`createPiHarness`, shared model/context/event runners, tool factories).
- `test/helpers/cursor-provider-harness.ts` owns Cursor SDK provider mocks/stream helpers and re-exports the OMP harness fixtures for provider tests.
- `docs/cursor-model-ux-spec.md` is the maintainer design source of truth for OMP Cursor model UX. Keep it aligned with behavior changes.
- `docs/cursor-testing-lessons.md` is the maintainer source of truth for regression testing lessons and isolated OMP smoke harnesses.
- `docs/cursor-dogfood-checklist.md` is the minimal one-session OMP dogfood checklist (baseline env, JSONL ID patterns, bootstrap manifest, edit diff card).
...
- Keep OMP-native abstractions first: context is a model variant, thinking uses OMP thinking metadata, and Cursor-only `fast` is extension state/status.
...
- Install dependencies: `bun install` (runs `prepare`, which compiles `src/` into `dist/` — the manifest entry OMP loads)
- Build after editing `src/`: `npm run build` — required before any direct `omp -e .` run, or OMP loads the previous build. The cloud/steering/local-resume/provider-debug launchers rebuild automatically (even when run directly with `node scripts/...`), `smoke:live`/`smoke:visual`/`smoke:isolated` build via their package scripts, and `smoke:platform*` builds inside its packed installs; only direct `omp -e .` runs need a manual build.
- Run tests: `npm test`
...
- Local development run, requires a Cursor key: `CURSOR_API_KEY="your-key" omp --auto-approve -e . --model cursor-sdk/grok-4.6`
- List Cursor models, requires OMP and usually a Cursor key: `omp models cursor-sdk`
...
- Do not commit `dist/`, `coverage`, `.env*`, `.omp/`, or package tarballs.
...
- NEVER store Cursor API keys in repo files, project `.omp/cursor-sdk.json`, tests, logs, snapshots, or docs examples. Protect the global `~/.omp/agent/cursor-sdk.json` with user-only permissions and never print or commit it.
- Scrub Cursor SDK errors and output that may contain API keys, bearer tokens, cookies, sessions, or auth headers.
- `PI_CURSOR_SDK_EVENT_DEBUG=1` and `npm run debug:provider-events` write raw local artifacts that may contain prompts, tool args/results, local paths, or secrets; keep them under gitignored `.debug/`, do not print or commit them, and keep run-scoped debug state explicit rather than process-global.
- Ambient Cursor settings/rules loading is enabled by default through `PI_CURSOR_SETTING_SOURCES=all`; keep OMP SDK startup log filtering intact so settings/skills output does not corrupt the OMP TUI. Users can narrow or disable Cursor setting sources explicitly when desired.
- Live OMP/Cursor smoke tests may call external services and require `CURSOR_API_KEY`; run them for Cursor provider/runtime changes when auth is available. If auth is unavailable, report live smoke as not run, and keep release-ready claims blocked. No `auth.json` is read or seeded by this extension.
- For live runtime evidence, use `cursor-sdk/grok-4.6:slow` as much as needed. If Cursor Cloud does not support that exact model variant, use `cursor-sdk/grok-4.6`.
...
- `npm run smoke:platform:all` is the canonical local runtime **release** gate for Cursor provider/runtime changes; ordinary OMP adaptation commits use the `## Verification lanes` below. Cloud runtime changes additionally require the opt-in `npm run smoke:cloud` lane. The platform gate uses packed installs across macOS, Ubuntu, and Windows native with PTY/ConPTY capture, host-rendered xterm/PNG visual evidence, JSONL assertions, bridge diagnostics, usage/cache checks, abort cleanup, artifact manifests, and redaction scans.
...
Routine OMP adaptation commits that change provider/runtime, prompt/session wiring, bridge, replay, or related extension code:

- Run `npm run build`, `npm run typecheck`, and `npm test`.
- Run `npm pack --dry-run` when package metadata, agent manifests, dependencies, publishable docs, or ignored artifacts change.
- Run one real OMP native task smoke for Cursor provider/runtime changes when temporary Cursor auth is available. Pass `CURSOR_API_KEY` only through the process environment; never write it to repo files, auth DB, logs, or docs.
- `npm run smoke:platform:all` is the release/platform gate, not a routine OMP adaptation commit requirement. Run it before release-ready status, tagging, publishing, or claiming macOS/Ubuntu/Windows behavior.
- Run `npm run smoke:cloud` only when the change exercises actual Cursor Cloud runtime execution.

The routine commit gate is build, typecheck, tests, package validation when applicable, and the targeted OMP smoke. The full platform matrix remains mandatory for release claims.
...
- Dependencies install with `bun install` (this triggers `prepare`, which compiles `src/` to `dist/`; the OMP manifest loads `dist/index.js`). After editing `src/`, run `npm run build` before any `omp -e .` run or the extension loads the previous build.
- OMP: use Bun `>=1.3.14` and Node `>=22.19.0` for the maintained scripts. Older runtimes are unsupported.
- `CURSOR_API_KEY` is provided as a cloud-agent secret, so live Cursor runs and full live model discovery work without any OMP login storage. `npm test`, `npm run typecheck`, and `npm pack --dry-run` need no key.
- Run the extension locally with `omp -e . --model cursor-sdk/grok-4.6` (the bare `omp` may not be on `PATH`). Add `--auto-approve` for interactive sessions; print-mode smoke: `omp -e . --model cursor-sdk/grok-4.6 --cursor-no-fast --no-session -p "..."`
- Cold-start gotcha: the first Cursor SDK run in a fresh VM can take several minutes (SDK/transport warm-up); subsequent runs complete in ~10s. Warm up with one throwaway run before timing-sensitive or recorded demos, and do not treat a slow first run as a hang.
...
- Use sessionful runs with `--session-dir` (not `--no-session`) when testing session ledgers, resume identity, branch/fork/clone/switch behavior, or slash commands such as `/cursor-cloud`; use `--continue` or `--resume` for a later process. `--no-session` is only proof for one-shot provider behavior.
- For slow cloud or slash-command probes, prefer print mode or raw JSONL RPC with an explicit timeout; a fixed 30s request timeout can falsely fail long cloud operations.
- Basic setup validation here is unit/typecheck/print-mode only. `npm run smoke:platform:all` remains the maintainer local-runtime release/pre-commit gate and needs the full macOS/Ubuntu/Windows matrix hosts; a Linux-only cloud agent cannot satisfy that gate. Treat Linux-only visual/resume/doctor results as partial evidence, not release-ready.
- Visual smoke (`npm run smoke:visual`) needs `omp` on `PATH` and Playwright Chromium (`bunx playwright install chromium`) for PNG capture; use `--no-screenshot` if Chromium is unavailable.
- `npm run smoke:live` needs `omp` on `PATH`. Prefer the package's `.bin` directory on `PATH` rather than relying on a global install.
