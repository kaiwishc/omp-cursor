...
describe("streamCursor stream events", () => {
	...
	it("detects trailing user messages only after tool results", () => {
		...
		it("serializes concurrent Cursor turns for one pi session across model selections", async () => {
			...
			const secondEventsPromise = collectEvents(streamCursor(makeModel("gpt-5.5@272k"), makeContext([{ role: "user", content: "second task", timestamp: 1 }]), { apiKey: "test-key", reasoning: "medium" as Effort }));
		...
		it("aborts a queued Cursor turn without waiting for the active turn", async () => {
			...
			const secondEventsPromise = collectEvents(streamCursor(makeModel("gpt-5.5@272k"), makeContext([{ role: "user", content: "second task", timestamp: 1 }]), { apiKey: "test-key", reasoning: "medium" as Effort, signal: controller.signal }));
			...
			const thirdEventsPromise = collectEvents(streamCursor(makeModel("gpt-5.5@272k"), makeContext([{ role: "user", content: "third task", timestamp: 1 }]), { apiKey: "test-key", reasoning: "medium" as Effort }));
