...
function printHelp() {
	console.log(`Canonical offscreen OMP TUI visual smoke runner for omp-cursor.
...
Common options:
  ...
  --out-dir PATH                Artifact directory. Default: /tmp/omp-cursor-visual-smoke-<timestamp>.
...
Native replay isolation defaults:
  ...
  PI_CODING_AGENT_DIR=<out-dir>/omp-agent  (seeded cursor-sdk.json + quietStartup; host extensions stay out)
  PI_OFFLINE=1
...
function parseArgs(argv) {
	...
	options.outDir ??= resolve(`/tmp/omp-cursor-visual-smoke-${timestamp()}`);
...
function seedVisualAgentDir(agentDir) {
	...
	const srcConfig = join(homedir(), ".omp", "agent", "cursor-sdk.json");
	const destConfig = join(agentDir, "cursor-sdk.json");
	try {
		copyFileSync(srcConfig, destConfig);
		chmodSync(destConfig, 0o600);
	} catch {
		// CURSOR_API_KEY remains the fallback when the global OMP config is absent.
	}
	writeFileSync(
