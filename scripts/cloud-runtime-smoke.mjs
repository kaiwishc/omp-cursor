...
const MODEL = "cursor-sdk/default";
...
function printHelp() {
	console.log(`Required live Cursor cloud release smoke for omp-cursor.
...
async function runBranchAndLifecycleLane({ artifactRoot, repo, timeoutMs, Agent }) {
	...
	const rpc = await startRpc({ artifactDir, sessionId, continueSession: true, envOptions });
	try {
...
async function main() {
	...
	const artifactRoot = mkdtempSync(join(tmpdir(), "omp-cursor-cloud-smoke-"));
