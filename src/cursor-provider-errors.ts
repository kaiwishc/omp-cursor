...
const GENERIC_CURSOR_SDK_ERROR_MESSAGE =
	"Cursor SDK request failed. The Cursor SDK API key may be missing, invalid, or unauthorized. Verify CURSOR_API_KEY or apiKey in ~/.omp/agent/cursor-sdk.json, or pass --api-key to OMP, then retry.";
const AUTH_CURSOR_SDK_ERROR_MESSAGE =
	"Cursor SDK request failed because the Cursor SDK API key may be invalid or unauthorized. Verify CURSOR_API_KEY or apiKey in ~/.omp/agent/cursor-sdk.json, or pass --api-key to OMP, then retry.";
const CLOUD_AUTH_CURSOR_SDK_ERROR_MESSAGE =
	"Cursor Cloud Agents request failed because Cloud API authentication rejected the API key. Use a user API key from Cursor Dashboard -> API Keys or a service account API key from Team settings; Team Admin API keys are not supported as Cursor Cloud Agents credentials. Configure CURSOR_API_KEY, apiKey in ~/.omp/agent/cursor-sdk.json, or --api-key to OMP, then retry.";
...
function isCursorExtensionConnectStack(stack: string): boolean {
	...
	// connect-web is the SDK's Bun/Deno path and is intentionally not classified
	// for supported OMP runs.
	return stack.includes("@connectrpc/connect-node") && /(?:^|[\\/])omp-cursor(?:[\\/]|$)/.test(stack);
}
