...
export const CURSOR_PI_BRIDGE_PREFERENCE_TEXT =
	"When exposed, prefer pi__mcp for MCP work and pi__task for delegation; use Cursor-configured MCP or Cursor-native subagents only when the matching pi__ tool is not exposed or unavailable.";
...
const CURSOR_PI_BRIDGE_CONTRACT_LINES = [
	...
	"Bridged calls execute through normal OMP tool flow, so OMP shows the real OMP tool name and returns a normal OMP tool result.",
